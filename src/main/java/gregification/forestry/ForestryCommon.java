...
import gregification.forestry.recipes.MiscRecipes;
...
public class ForestryCommon {
    public static boolean EXTRA_BEES = ForestryConfig.extraBees && Loader.isModLoaded(ModIDs.MODID_EB);
    public static boolean TWILIGHT_BEES = ForestryConfig.twilightBees && Loader.isModLoaded(ModIDs.MODID_TF) && Loader.isModLoaded(ModIDs.MODID_MB);
    public static boolean THAUMIC_BEES = ForestryConfig.thaumicBees && Loader.isModLoaded(ModIDs.MODID_THAUM) && Loader.isModLoaded(ModIDs.MODID_MB);
    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        if (ForestryConfig.gtBees) {
            if (ForestryUtils.apicultureEnabled()) {
                ...
                ForestryModule.logger.error("GT Bees is enabled, but Forestry Apiculture module is disabled. Skipping GT Bees...");
            }
        }
        ...
        if (ForestryConfig.gtFrames) {
            if (ForestryUtils.apicultureEnabled()) {
                ...
                ForestryModule.logger.error("GT Frames is enabled, but Forestry Apiculture module is disabled. Skipping GT Frames...");
            }
        }
    }
    ...
    public void init(FMLInitializationEvent event) {
        ...
        if (ForestryConfig.gtElectrodes) {
            ForestryModule.logger.info("Registering Forestry Electrode recipe overrides");
        ...
        if (ForestryUtils.apicultureEnabled()) {
            if (ForestryConfig.gtBees) {
                ForestryModule.logger.info("Registering GT Bee alleles, species and mutations");
    ...
    public void postInit(FMLPostInitializationEvent event) {
        ...
        if (ForestryUtils.apicultureEnabled()) {
            ForestryModule.logger.info("Registering Forestry Comb recipe overrides");
    ...
    public void registerItems(RegistryEvent.Register<Item> event) {
        ...
        if (ForestryUtils.apicultureEnabled()) {
            if (ForestryConfig.gtBees) {
                ForestryModule.logger.info("Registering GT Comb and Drop items");
                event.getRegistry().register(gtCombs);
                event.getRegistry().register(gtDrops);
            }
            if (ForestryConfig.gtFrames) {
                ForestryModule.logger.info("Registering GT Frame items");
        ...
        if (ForestryConfig.gtElectrodes) {
            ForestryModule.logger.info("Registering GT Electron Tubes");
        ...
        if (ForestryConfig.gtScoop) {
            ForestryModule.logger.info("Registering GT Scoop");
    ...
    public void registerRecipes(RegistryEvent.Register<IRecipe> event) {
        ...
        if (ForestryConfig.gtElectrodes) {
            ForestryModule.logger.info("Registering GT Electrode recipes");
        ...
        if (ForestryUtils.apicultureEnabled()) {
            ...
            if (ForestryConfig.gtBees) {
                ForestryModule.logger.info("Registering GT Bee, Comb, Drop and Propolis recipes");
            ...
            // GT Frame recipes
            if (ForestryConfig.gtFrames) {
                ForestryModule.logger.info("Registering GT Frame recipes");
                FrameRecipes.init();
            }
        }
    }
    ...
    public void registerModels(ModelRegistryEvent event) {
        if (ForestryUtils.apicultureEnabled()) {
            if (ForestryConfig.gtBees) {
                ForestryCommon.gtCombs.registerModel(ForestryCommon.gtCombs, ForestryAPI.modelManager);
                ForestryCommon.gtDrops.registerModel(ForestryCommon.gtDrops, ForestryAPI.modelManager);
            }
            if (ForestryConfig.gtFrames) {
                ForestryCommon.gtFrames.values().forEach(f -> f.registerModel(f, ForestryAPI.modelManager));
            }
        }
    }

    public void registerMaterials(GregTechAPI.MaterialEvent event) {
        ForestryModule.logger.info("Registering Material additions");
